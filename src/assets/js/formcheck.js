...
export default {
  install: function(Vue){
    ...
    Vue.prototype.emailTest = function (txt) {
      return/@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9]+)+$/.test(txt)
    },
    // 检测固话号码
    Vue.prototype.teleTest = function (txt) {
      return /^((0\d{2,3}-\d{7,8}))$/g.test(txt)
    },
    // 检测手机和固话
    Vue.prototype.telephoneTest = function (txt) {
      return /^((0\d{2,3}-\d{7,8})|(1[345678]\d{9}))$/g.test(txt)
    },
    // 隐藏手机号
    Vue.prototype.phoneChang = function (txt) {
    ...
    // 验证码
    Vue.prototype.setTime = function (that) {
      if (that.phonecode.phoneFlag === 0) {
        ...
        var times = setInterval(function () {
          if (--that.phonecode.time >= 0) {
            ...
            clearInterval(times)
            // that.time = 3
            that.phonecode.phoneText = '获取验证码'
            that.phonecode.phoneFlag = 0
          }
        }, 1000)
        that.phonecode.oldPhone = that.frontend.phone // 记录现时的手机号
      }
    }
  }
}
